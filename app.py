...
@st.cache_data
def load_data():
    paths = ["四家七年.csv", "1234/四家七年.csv", "kent/1234/四家七年.csv"]
...
if data is not None:
    ...
    with st.sidebar:
        ...
        benchmark_name = next((c for c in all_cols if "元大台灣50" in c or "0050" in c), None)
    ...
    for j in range(1, len(m_ret)):
        ...
        if prev_corr < threshold:
            ...
            action, strat_ret = (f"買{s2}/賣{s1}", r2 - r1) if r1_p > r2_p else (f"買{s1}/賣{s2}", r1 - r2)
        
        results.append({"月份": curr_dt.strftime('%Y-%m'), "相關性": prev_corr, "交易動作": action, "策略獲利": strat_ret})
    ...
    c1.metric("累積淨獲利", f"{res_df['累積報酬'].iloc[-1]*100:.2f}%", 
              delta=f"{res_df['策略獲利'].iloc[-1]*100:.2f}%", delta_color="inverse")
    c2.metric("年化夏普比率", f"{sharpe:.2f}")
    c3.metric("累積最高收益", f"{max_cum_profit*100:.2f}%")
    ...
    with tab1:
        ...
        if show_benchmark:
            ...
            fig.add_trace(go.Scatter(x=res_df['月份'], y=b_cum.values[:len(res_df)]*100, 
                                     name='元大台灣50', line=dict(color='#64748B', dash='dot')))
    ...
    with tab3:
        ...
        fig_corr.update_layout(yaxis_range=[-1, 1], template="plotly_white")
        st.plotly_chart(fig_corr, width="stretch")
else:
    st.info("請檢查 CSV 檔案路徑與內容。")